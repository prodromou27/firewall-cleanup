#!/usr/bin/env bash
#
# PolicyInsight — bare-metal in-place update. Pulls this host's branch (DEV on
# the DEV box, PROD on the PROD box), updates deps, runs migrations, rebuilds the
# frontend, and restarts services. Used directly or by the self-update timer.
#
...
if [ "$ok" = "1" ]; then
  ...
  warn "Health check failed. Inspect / roll back:"
  echo "    journalctl -u policyinsight-backend.service --no-pager -n 80"
  echo "    git reset --hard ${BEFORE} && sudo bash deploy/update.sh"
  exit 1
fi
