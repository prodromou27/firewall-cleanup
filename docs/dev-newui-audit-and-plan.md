...
Reanalysis commits finding replacement, rule scores and completion metadata in one transaction. Detector or database failures roll back the replacement, preserving the previous findings, reviewer comments and scores, while recording the failed run separately. Successful reanalysis now archives every replaced finding, workflow field and comment in `AnalysisRun.replaced_findings`. The tenant-authorized, read-only `GET /api/policies/{policy_id}/review-history` endpoint exposes paginated run snapshots. New findings require review; previous approvals are not automatically applied to changed evidence. Stable finding reconciliation and a dedicated history UI remain future work. History deleted before this change cannot be reconstructed.

Revision `f8a0c3d5e7b9` adds the nullable JSON history column. Upgrade with `alembic upgrade head` before running this backend version. The migration was rehearsed on a temporary database at the previous revision with existing customer, policy and analysis-run rows; all were preserved and older runs retain null history. No customer database was migrated or reanalyzed during this review.

Additional false-positive controls: consolidation requires adjacent rules with matching modeled schedule, identity, application, NAT, logging, security-profile and evaluation-context fields; negated rules are excluded. Schedule names no longer generate expiration findings without resolved dates and recurrence. Internal-destination and broad-internal checks require concrete RFC1918 address space; special-use addresses do not prove public exposure. Missing explicit cleanup rules are informational observations, not proof of an implicit-deny logging gap. Logging findings require an enabled allow rule and an explicitly disabled logging setting.
...
Duplicate-object detection and the scorecard now share a comparison of vendor, object type, exact trimmed value, protocol, ports and retained raw data. Service comparisons require TCP/UDP and valid explicit port bounds; incomplete or unsupported service definitions are suppressed. Only `source_ref` is excluded from raw-data comparison. Differences in raw identity or descriptive fields can therefore suppress otherwise equivalent objects: this is intentionally conservative until vendor semantics are modeled explicitly. These findings remain review candidates, not approval to consolidate objects.
